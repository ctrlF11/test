/*member_join.js*/
  /* 기본 회원정보 체크 */
	function mem_check(){
	  	...
	  	if($.trim($("#birth_year").val())==""){
	  		alert("년도를 입력하세요!");
	  		$("#birth_year").val("").focus();
	  		return false;
	  	}
	  	if($.trim($("#birth_month").val())==""){
	  		alert("월을 입력하세요!");
	  		$("#birth_month").val("").focus();
	  		return false;
	  	}
	  	...
	  	if($.trim($("#birth_month").val())=="2" && $.trim($("#birth_day").val()) > 29){
	  		alert("2월은 30일 이상이 없습니다.");
	  		$("#birth_day").val("").focus();
	  		return false;
	  	}
	  	
	  	if($.trim($("#member_mail1").val())==""){
	  		alert("이메일 주소를 입력해주세요!");
	  		$("#member_mail1").val("").focus();
	  		return false;
	  	}
	  	if($.trim($("#member_mail2").val())=="" && $.trim($("#member_mail3").val())==""){
	  		alert("이메일 도메인을 입력해주세요!");
	  		$("#member_mail3").val("").focus();
	  		return false;
	  	}
	  	...
	  	if($.trim($("#password_question").val())==""){
	  		alert("비밀번호 힌트를 선택하세요!");
	  		$("#password_question").val("").focus();
	  		return false;
	  	}
	  	if($.trim($("#password_answer").val())==""){
	  		alert("비밀번호 힌트를 선택하세요!");
	  		$("#password_answer").val("").focus();
	  		return false;
	  	}
	  }
  ...
  function post_checks(){
	   	window.open("zipcode_find.do", "우편번호 주소 찾기", "width=450, height=150");
