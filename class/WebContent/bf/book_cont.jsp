...
<div id="facil_article">
	<div id="reserveMainTop">
		<%
			bfVO vo = (bfVO)request.getAttribute("VO");
			bfVO vop = (bfVO)request.getAttribute("VOp");
			bfVO vom = (bfVO)request.getAttribute("VOm");
	...
	<div id="reserveMainBottom">
		<ul style="height: 15px;">
			<li>
				<input type="button" onclick="javascript:document.location.href='book_want.do?page=<%= backPage %>'" value="목록"
		...
		<ul style="text-align: left; margin-left: -20px;">
			...
			%>
			<li>
				△<a href="<c:url value='/bw_cont.do' />?no=<%= vop.getBr_no() %>&amp;page=<%= backPage %>" ><%= vop.getBr_title() %></a>
			</li>
			<%} %>
			<%
			...
			%>
			<li>
				▽<a href="<c:url value='/bw_cont.do' />?no=<%= vom.getBr_no() %>&amp;page=<%= backPage %>" ><%= vom.getBr_title() %></a>
			</li>
			<%} %>
		</ul>
