		...
		<div id="data_board_bottom">
			<ul style="text-align: center;">
				<%
					if(nowPage > block)
					{%>
						[<a href="admin_manage.do?page=1">◀◀</a>]
						[<a href="admin_manage.do?page=<%=startPage-1%>">◀</a>]
					<%}	%>
				<%
					for(int i=startPage;i<=endPage;i++)
					{
						...
						{%>
							<b>[<%=i %>]</b>
						<%} else { %>
							[<a href="admin_manage.do?page=<%=i%>"><%=i %></a>]
					<%}
					}%>
				<%	
					if(endPage < allPage)
					{%>
						[<a href="admin_manage.do?page=<%=endPage+1%>">▶</a>]
						[<a href="admin_manage.do?page=<%=allPage%>">▶▶</a>]
				<%} %>
			</ul>	
		</div>
	</div>
